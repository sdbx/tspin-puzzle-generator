...
class Board {
    ...
    compareAt(y: number, x: number, value: Board, ignoreZero = true) {
    ...
    fillAt(y: number, x: number, value: Board, ignoreZero = true, inv = false) {
        for (let i = 0; i < value.h; i++) {
            for (let j = 0; j < value.w; j++) {
                if (ignoreZero && value.get(i, j) === 0) {
                    continue;
    ...
    getFits(board: Board, ignoreZero = true) {
        const output = [];
